...
import { Plus, Pencil, Trash2, Download, ArrowRightLeft, Send, ArrowLeft } from 'lucide-react'
...
const exportDevisPDF = async (
  ...
  if (!full) return
  const { data: signature } = await supabase.from('devis_signatures').select('*').eq('devis_id', devis.id).eq('statut', 'signé').maybeSingle()
  ...
  if (signature) {
    y += 6
    const dateSignature = new Date(signature.signe_le).toLocaleString('fr-FR', { dateStyle: 'long', timeStyle: 'short' })
    doc.setFillColor(220, 252, 231); doc.rect(14, y - 5, 182, 14, 'F')
    doc.setTextColor(22, 101, 52); doc.setFont('helvetica', 'bold'); doc.setFontSize(9)
    doc.text(`✓ DEVIS ACCEPTÉ ÉLECTRONIQUEMENT le ${dateSignature}`, 18, y)
    doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5)
    doc.text(`Adresse IP de validation : ${signature.ip_signature ?? 'non disponible'}`, 18, y + 5)
    y += 16
  }
...
export default function DevisClientPage() {
  ...
  const fetchData = async () => {
    ...
    const { data } = await supabase.from('devis').select('*, clients(id, nom, email)').eq('client_id', clientId).order('created_at', { ascending: false })
  ...
  useEffect(() => { Promise.all([fetchData(), fetchClients()]).then(() => setLoading(false)) }, [clientId])
  ...
  const handleSubmit = async (formData: DevisFormData) => {
    ...
    setDialogOpen(false); setEditing(null); await fetchData()
  }
  ...
  const ActionButtons = ({ devis }: { devis: Devis }) => (
    <div className="flex items-center gap-1">
      <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-500 hover:text-blue-600 hover:bg-blue-500/10" title="Envoyer par email" onClick={() => envoyerDevisParEmail(devis, fetchData)}>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => router.push('/devis')}>
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) setEditing(null) }}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <DevisForm devis={editing ?? ({ client_id: clientId } as DevisAvecLignes)} clients={clients} onSubmit={handleSubmit} onCancel={() => { setDialogOpen(false); setEditing(null) }} />
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={(open) => { if (!open) setDeleteConfirm(null) }}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">Cette action est irréversible. Le devis et ses lignes seront définitivement supprimées.</p>
