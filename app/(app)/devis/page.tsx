...
const exportDevisPDF = async (devis: Devis, supabase: ReturnType<typeof createClient>) => {
  ...
  const totals = [
    ['Total HT', formatEur(devis.total_ht)],
    ['TVA', formatEur(devis.total_tva)],
...
const convertirEnFacture = async (
  ...
  // Récupérer le devis complet avec lignes
  const { data: full } = await supabase
    .from('devis')
    .select('*, devis_lignes(*)')
    .eq('id', devis.id)
  ...
  if (newFacture && full.devis_lignes?.length > 0) {
    await supabase.from('factures_lignes').insert(
      full.devis_lignes.map((l: { designation: string; quantite: number; prix_unitaire_ht: number; tva: number; ordre: number }, i: number) => ({
  ...
  alert(`✅ Facture ${numero} créée avec succès ! Rendez-vous dans l'onglet Factures.`)
  onSuccess()
}
...
export default function DevisPage() {
  ...
  useEffect(() => {
    Promise.all([fetchDevis(), fetchClients()]).then(() => setLoading(false))
  }, [])
  ...
  const openEdit = async (devis: Devis) => {
    const { data } = await supabase
      .from('devis')
      .select('*, devis_lignes(*), clients(id, nom, email)')
      .eq('id', devis.id)
      .single()
    if (data) {
    ...
    if (editing) {
      ...
      if (lignes.length > 0) {
        await supabase.from('devis_lignes').insert(
          lignes.map((l, i) => ({ ...l, devis_id: editing.id, ordre: i }))
        )
      }
    } else {
      ...
      if (newDevis && lignes.length > 0) {
        await supabase.from('devis_lignes').insert(
          lignes.map((l, i) => ({ ...l, devis_id: newDevis.id, ordre: i }))
        )
      }
    }
                ...
                <TableRow key={devis.id} className="group">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button
                        variant="ghost" size="icon" className="h-8 w-8 text-green-500 hover:text-green-600 hover:bg-green-500/10"
                        title="Convertir en facture"
                        onClick={() => convertirEnFacture(devis, supabase, fetchDevis)}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => setDeleteConfirm(devis.id)}
                      >
