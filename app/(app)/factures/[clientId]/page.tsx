...
export default function FacturesClientPage() {
  ...
  const fetchData = async () => {
    ...
    const { data } = await supabase.from('factures').select('*, clients(id, nom, email)').eq('client_id', clientId).order('created_at', { ascending: false })
  ...
  const handleSubmit = async (formData: FactureFormData) => {
    ...
    if (editing) {
      ...
      const { data: newFacture } = await supabase.from('factures').insert({ numero: factureData.numero, client_id: factureData.client_id || clientId, date_creation: factureData.date_creation, date_echeance: factureData.date_echeance, statut: factureData.statut, notes: factureData.notes, total_ht: factureData.total_ht, total_tva: factureData.total_tva, total_ttc: factureData.total_ttc }).select().single()
    ...
    <div className="space-y-6">
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) setEditing(null) }}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <FactureForm facture={editing ?? ({ client_id: clientId } as FactureAvecLignes)} clients={clients} onSubmit={handleSubmit} onCancel={() => { setDialogOpen(false); setEditing(null) }} />
        </DialogContent>
      </Dialog>
